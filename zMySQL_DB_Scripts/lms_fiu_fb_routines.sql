...
/*!50003 DROP PROCEDURE IF EXISTS `selectStudentByUserId` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8mb4 */ ;
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `selectStudentByUserId`(IN inUserId BIGINT)
BEGIN
	DECLARE C INT;
    
    SELECT COUNT(user_id) INTO C
    FROM students
    ...
    IF(C < 1) THEN
		SIGNAL SQLSTATE '45000'
			SET MESSAGE_TEXT = 'user_id not found in students table';
	END IF;
    
    SELECT * FROM students WHERE user_id = inUserId;
...
-- Dump completed on 2020-02-06 21:11:36
